...
const UpdateFeedbackForm = ({ course }) => {
  ...
  const handleAddStarCategory = () => {
    ...
    if (newCat && !starCategories.includes(newCat)) setStarCategories([...starCategories, newCat]);
  };

  const handleRemoveStarCategory = (cat) => {
    if (window.confirm("Remove this category?")) setStarCategories(starCategories.filter(c => c !== cat));
  };
  ...
  const handleAddOption = () => {
    ...
    const choices = prompt("Comma separated choices:")?.split(",").map(c => c.trim());
    if (!label || !choices?.length) return;
    setOptions([...options, { key, label, choices }]);
  };

  const handleRemoveOption = (key) => {
    if (window.confirm("Remove this question?")) setOptions(options.filter(o => o.key !== key));
  };

  const handleSaveForm = () => {
    localStorage.setItem("adminFeedbackForm", JSON.stringify({ starCategories, options }));
  ...
  return (
    <div className="feedback-container">
      ...
      <div className="feedback-form">
        ...
        <button className="save-btn" onClick={handleSaveForm}>💾 Save Changes</button>
      </div>
